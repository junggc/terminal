		...
		if(isset($third_menu[$ka][$a])){	
			foreach($third_menu[$ka][$a] as $c => $d){
				...
				if(preg_match("/\/html\//", $d)){		
					...
					if(!preg_match("/\.php/", $d)) exit("확장자가 .php가 아닙니다.");
				
					$new_url = str_replace(G5_THEME_URL,"",$d);
					$new_url = str_replace("/html/","",$new_url);
					$new_url_arr = @explode("/",$new_url);
			...
			foreach($third_menu[$ka][$a] as $c => $d){
				...
				if(preg_match("/\/html\//", $d)){				
					...
					if($config['cf_bbs_rewrite'] == "1") $a_url[] = $new_url;
						...
						$a_url[] = $new_url."/".preg_replace("/\s/",'-',$c)."/";
					}
					echo "<tr><td width=120>".$c."</td><td>G5_URL.\"".implode('/',$a_url)."\"</td></tr>";
				}
				unset($a_url);	
			}
		}
		...
		if(isset($third_menu[$ka][$a])){	
			foreach($third_menu[$ka][$a] as $c => $d){
				if(preg_match("/\/html\//", $d)){				
					...
					echo "<tr><td width=120>".$c."</td><td><a href='".G5_URL."".implode('/',$a_url)."' target='_blank'>".G5_URL."",implode('/',$a_url)."</a></td></tr>";
				}
				unset($a_url);	
			}
		}
