<?php
	include base64_decode($_GET['gurl'])."/common.php";
	if (!defined('_GNUBOARD_')) exit; // 개별 페이지 접근 불가
	/*
		
		이 파일을 절대 수정, 삭제하지 마세요.
	
	*/
	$OPTIMIZE_VER = 3;
	$check_keys = array('fname', 'type', 'token', 'skin_dir', 'gurl');
	$_GETA = $_GET;

	foreach($_GETA as $k => $v){
	...
	if($nowt != $ex_token[1]){ $_SERVER['HTTP_REFERER'] = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';	//로딩속도가 다를때 
	...
	$fname = "style.dt_ori.".$fname;
	$fname = $fname.".".$type;

	$f_file = base64_decode($skin_dir)."/".$fname;
	...
	foreach($listc as $k => $v){	
		...
		$strz = str_replace(")","",$strz);
		$temp = parse_url($strz);    $temp['host'] = isset($temp['host']) ? $temp['host'] : '';
		if($temp['host']){
			$temp_url[] = $temp['host']; 
		}
	}
