...
//echo "REDIRECT_URL ".($_SERVER['REDIRECT_URL'])."<BR><BR>";
...
function daontheme_lastest($skin_dir='', $bo_table, $rows=10, $subject_len=40, $cache_time=1, $options='')
{
    ...
    if( $caches == false ){
        ...
        $sql_ = "";   if($daon_category) $sql_ = " and ca_name = '{$daon_category}' ";
...
if($config['cf_bbs_rewrite'] != "0"){
	foreach($first_menu as $a => $b){				
		...
		$daon_url_exp = @explode("&",isset($daon_url['query']) ? $daon_url['query'] : '');		
		$daon_url_exp_query = isset($daon_url['query']) ? $daon_url['query'] : '';
	...
	foreach($second_menu as $ka => $va){
		foreach($second_menu[$ka] as $a => $b){	
			...
			if(isset($third_menu[$ka][$a])){	
				foreach($third_menu[$ka][$a] as $c => $d){
					$bo_table_re ="";
					$co_id_re ="";
					$daon_url = parse_url($d); 
					$daon_url_exp = @explode("&",isset($daon_url['query']) ? $daon_url['query'] : '');
					$daon_url_exp_query = isset($daon_url['query']) ? $daon_url['query'] : '';
					$daon_url_exp_a = $daon_url['path'];
					...
					foreach($daon_url_exp as $y => $z){				
						if(preg_match("/bo_table\=/",$z)){				
							if(preg_match("/write\.php/",$daon_url_exp_a)){	
								$bo_table_re = str_replace("bo_table=","",$z);				
								$third_menu[$ka][$a][$c] = urldecode(get_pretty_url($bo_table_re)."/write"."?".$daon_url_exp_query);
							}else{
								$bo_table_re = str_replace("bo_table=","",$z);				
								$third_menu[$ka][$a][$c] = urldecode(get_pretty_url($bo_table_re)."?".$daon_url_exp_query);
							}
						}
						if(preg_match("/co_id\=/",$z)){						
							$co_id_re = str_replace("co_id=","",$z);				
							$third_menu[$ka][$a][$c] = urldecode(get_pretty_url('content', $co_id_re));
						}				
					}	
					if(preg_match("/\/html\//",$d)){		
						
						$new_url_exp = explode(G5_THEME_DIR."/".$config['cf_theme'],$d);			
						$new_url = str_replace(G5_THEME_DIR."/".$config['cf_theme'],"",$d);
						$new_url = str_replace("/html/","".str_replace("daontheme_","",$config['cf_theme'])."/",$new_url);		
						$new_url = preg_replace('/\.[^.]+$/','',$new_url);
						$new_url = str_replace($new_url_exp[0],"/",$new_url);
						$new_url = G5_URL.$new_url;	

						if($config['cf_bbs_rewrite'] == "1") $third_menu[$ka][$a][$c]  = $new_url;
						else if($config['cf_bbs_rewrite'] == "2"){
							$third_menu[$ka][$a][$c]  = $new_url."/".preg_replace("/\s/",'-',$c)."/";
						}	
					}
					unset($daon_url_exp);
				}
...
$tmenu_ = $tmenu2_ = "";    $_SERVER['REDIRECT_URL'] = isset($_SERVER['REDIRECT_URL']) ? $_SERVER['REDIRECT_URL'] : '';
	...
	if($config['cf_bbs_rewrite'] != "0" && $_SERVER['REDIRECT_URL'] && ( preg_match("/\/bbs\/board.php/", $_SERVER['PHP_SELF']) ||  preg_match("/\/bbs\/write.php/", $_SERVER['PHP_SELF'])) ){ 
		...
		foreach($second_menu as $ka => $va){
			foreach($second_menu[$ka] as $a => $b){
				...
				if(isset($third_menu[$ka][$a])){	
					foreach($third_menu[$ka][$a] as $c => $d){
						if((preg_match("/\/".$bo_table."\/write/", $d) || preg_match("/\/".$bo_table."/", $d)) && !preg_match("/bbs\/qalist/", $d) && !preg_match("/bbs\/qawrite/", $d) && !preg_match("/bbs\/qaview/", $d)){				
							$tmenu_ = $ka;			
							$tmenu2_ = $a;									
							$g5['so_title']	= $third_menu_so_title[$ka][$a][$c];	
						}	
					}
				}
			}
		...
		foreach($first_menu as $a => $b){				
			$bo_table_re ="";
			$url = parse_url($b); 
			$daon_url_exp = @explode("&", isset($url['query']) ? $url['query'] : '' );			
			foreach($daon_url_exp as $y => $z){				
				if(preg_match("/bo_table\=/",$z)){				
					$bo_table_re = str_replace("bo_table=","",$z);				
					...
					foreach($third_menu[$ka][$a] as $c => $d){
						...
						foreach($daon_url_exp as $y => $z){			
							if(preg_match("/bo_table\=/",$z)){
								$bo_table_re = str_replace("bo_table=","",$z);
								if(preg_match("/sca\=/",$z)){
									$sca_re = str_replace("sca=","",$z);
								}
								if($sca_re){
									if($bo_table == $bo_table_re && $sca == $sca_re){
										$tmenu_ = $ka;							
										$tmenu2_ = $a;
										$g5['so_title']	= $third_menu_so_title[$ka][$a][$c];				
									}
...
$co_seo_title = isset($co_seo_title) ? $co_seo_title : '';
$co_id = isset($co_id) ? $co_id : '';
$co_id_a = isset($co_id_a) ? $co_id_a : '';
				...
				if(isset($third_menu[$ka][$a])){
					foreach($third_menu[$ka][$a] as $c => $d){
						...
						if($config['cf_bbs_rewrite'] == 1){
							...
							if(preg_match("/\/".($co_row['co_seo_title'])."\/$/", $d)){				
								$tmenu_ = $ka;		
								$tmenu2_ = $a;	
								$g5['so_title']	= $third_menu_so_title[$ka][$a][$c];			
							}	
...
if(preg_match("/bbs\/search\.php/",$_SERVER['PHP_SELF'])){
	...
	foreach($second_menu as $ka => $va){
		foreach($second_menu[$ka] as $a => $b){	
			...
			if(isset($third_menu[$ka][$a])){
				foreach($third_menu[$ka][$a] as $c => $d){
					if(preg_match("/bbs\/search\.php/",$d)){
						$tmenu_ = $ka;						
						$tmenu2_ = $a;
						$g5['so_title']	= $third_menu_so_title[$ka][$a][$c];
					}					
...
if(preg_match("/bbs\/member_confirm\.php/",$_SERVER['PHP_SELF'])){
	...
	foreach($second_menu as $ka => $va){
		foreach($second_menu[$ka] as $a => $b){	
			...
			if(isset($third_menu[$ka][$a])){
				foreach($third_menu[$ka][$a] as $c => $d){
					if(preg_match("/bbs\/member_confirm\.php/",$d)){
						$tmenu_ = $ka;						
						$tmenu2_ = $a;
						$g5['so_title']	= $third_menu_so_title[$ka][$a][$c];
					}					
...
if($second_menu_view == false){
	unset($second_menu, $second_menu_target);
}

if($third_menu_view == false){
	unset($third_menu, $third_menu_target);
}
